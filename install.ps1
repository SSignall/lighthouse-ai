...
Write-Host "  Local Claw Plus Session Manager - Windows Installer" -ForegroundColor Cyan
Write-Host "===========================================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $ProxyOnly) {
    ...
    $cleanupContent = @"
# Local Claw Plus Session Manager - Session Cleanup (Windows)
...
"@
    ...
    Register-ScheduledTask -TaskName $CleanupTaskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "Local Claw Plus Session Manager - Cleanup every ${IntervalMinutes}min" | Out-Null
